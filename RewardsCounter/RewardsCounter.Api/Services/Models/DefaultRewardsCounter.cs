...
public class DefaultRewardsCounter : IRewardsCounter
{
    ...
    public Reward CountReward(IEnumerable<Transaction> transactions, Customer customer, DateTime start, DateTime end)
    {
        ...
        if (start > end)
        {
            throw new ArgumentOutOfRangeException(nameof(start), "Start value in period can not be less then end.");
        }

        if (start < customer.CreationDate)
        {
            throw new ArgumentOutOfRangeException(nameof(start), "Start value can not be greater then customer registration date.");
        }
